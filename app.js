...
import { getFirestore, collection, doc, getDoc, getDocs, addDoc, setDoc, updateDoc, where, query, serverTimestamp, increment } from 'https://www.gstatic.com/firebasejs/11.6.1/firebase-firestore.js';
...
onAuthStateChanged(auth, (user) => {
  if (user) {
    ...
    document.getElementById("setOldBalancesSection").style.display = "none";
    document.getElementById("setInitialBalanceSection").style.display = "none";
  }
});
  ...
  try {
    ...
    if (userDoc.exists()) {
      ...
      if (userData.role === "treasurer" || userData.role === "caretaker") {
        ...
        document.getElementById("setOldBalancesSection").style.display = "block";
        document.getElementById("setInitialBalanceSection").style.display = "block";
        loadMembers();
        loadExpenses();
        loadMembersForOldBalances();
...
// Load Members for Setting Old Balances
async function loadMembersForOldBalances() {
  const memberBalanceList = document.getElementById("memberBalanceList");
  ...
  try {
    ...
    querySnapshot.forEach((doc) => {
      ...
      memberDiv.className = "member-balance";
      memberDiv.innerHTML = `
        <p>${userData.name}</p>
        <input type="number" id="balance_${doc.id}" placeholder="Enter old balance" value="${userData.balance || 0}" />
      `;
      memberBalanceList.appendChild(memberDiv);
    });
  } catch (error) {
    alert("Error loading members for old balances: " + error.message);
  }
}

// Save Old Balances
async function saveOldBalances() {
  const memberBalanceList = document.getElementById("memberBalanceList").children;
  try {
    for (const memberDiv of memberBalanceList) {
      const memberId = memberDiv.querySelector("input").id.split("_")[1];
      const balance = parseFloat(memberDiv.querySelector("input").value) || 0;
      await updateDoc(doc(db, "users", memberId), {
        balance: balance
      });
    }
    alert("Old balances saved successfully!");
    // Reload user data to update balance display
    const user = auth.currentUser;
    if (user) fetchUserData(user.uid);
  } catch (error) {
    alert("Error saving old balances: " + error.message);
  }
}

// Set Initial Society Balance
async function setInitialBalance() {
  const initialCash = parseFloat(document.getElementById("initialCash").value) || 0;
  const initialBank = parseFloat(document.getElementById("initialBank").value) || 0;
  ...
  try {
    ...
    document.getElementById("societyCash").innerText = initialCash;
    document.getElementById("societyBank").innerText = initialBank;
    document.getElementById("initialCash").value = "";
    document.getElementById("initialBank").value = "";
    alert("Initial society balance set successfully!");
  } catch (error) {
    ...
    .then((querySnapshot) => {
      ...
      querySnapshot.forEach((doc) => {
        ...
        receiptDiv.innerHTML = `
          <p>Receipt Number: ${receipt.receiptNumber || "N/A"}</p>
...
async function createReceipt() {
  ...
  try {
    // Increment receipt counter and get receipt number
    const counterRef = doc(db, "counters", "receiptCounter");
    let receiptNumber = 1;
    const counterDoc = await getDoc(counterRef);
    if (counterDoc.exists()) {
      receiptNumber = counterDoc.data().count + 1;
    }
    await setDoc(counterRef, { count: receiptNumber }, { merge: true });
...
document.getElementById("saveOldBalancesButton").addEventListener("click", saveOldBalances);
document.getElementById("setInitialBalanceButton").addEventListener("click", setInitialBalance);
Show in sidebar